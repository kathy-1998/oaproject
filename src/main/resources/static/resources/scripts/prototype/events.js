...
$axure.internal(function($ax) {
    ...
    var _handleEvent = $ax.event.handleEvent = function(elementId, eventInfo, axEventObject, skipShowDescriptions, synthetic) {
        ...
        if(currentEvent && currentEvent.handled && !eventInfo.isMasterEvent) return;
        ...
        if(currentEvent) {
            currentEvent.handled = !synthetic && !bubble && eventDescription != 'OnFocus';

            // Prevent default if necessary
            if(currentEvent.handled && preventDefaultEvents.indexOf(eventDescription) != -1) {
    ...
    var _showCaseDescriptions = function(elementId, eventInfo, axEventObject, synthetic) {
        ...
        if(!_isEventSimulating(axEventObject)) {
            for(var i = 0; i < axEventObject.cases.length; i++) {
                var $link = $("<div class='intcaselink' onmouseleave='$axure.legacy.SuppressBubble(event)'>" + axEventObject.cases[i].description + "</div>");
            ...
            var $link = $("<div class='intcaselink' onmouseleave='$axure.legacy.SuppressBubble(event)'>" + fullDescription + "</div>");
            $link.click(function() {
    ...
    var _showCaseLinks = function(eventInfo, linksId) {
        ...
        var body = $('body');
        if(body.css('position') == 'relative') left = (Number(left.replace('px', '')) - Number(body.css('left').replace('px', '')) - ($(window).width()-body.width())/2) + 'px';
    ...
    var _initializeObjectEvents = function(query) {
        ...
        query.filter(function(diagramObject) {
            ...
            $('#' + elementId).bind('change', function(e) {
                $ax.setjBrowserEvent(e);
                _tryFireCheckedChanged(elementId);
            });
        });
                ...
                $(query).bind('touchend', function(e) {
                    ...
                    if((xDiff * xDiff + yDiff * yDiff) < CLICK_THRESHOLD_PX_SQ) {
                        ...
                        if(time - lastClickEventTime < DBLCLICK_THRESHOLD_MS) {
                            ...
                            if(event.handled) e.preventDefault(); //for iOS
                        }
    ...
    var _updateMouseLocation = function(e, end) {
        ...
        if($.browser.msie) {
            _mouseLocation.x = e.clientX + $('html').scrollLeft();
            _mouseLocation.y = e.clientY + $('html').scrollTop();
        } else {
